/**
 * mornye-theme — host 半边（主题、壁纸、余额与外观开关，外加人格 preset 的幂等铺设）
 *
 ...
 *   GET  /plugins/mornye-theme/preset          → 莫宁人格 preset 的铺设状态（装没装、铺到哪里）
 *
 * 另外：apply() 时会把随包的 preset/ 幂等铺设到 $DSH_HOME/.agent-presets/mornye/（见文件中部的
 * deployPreset）。人格与壁纸是两件独立的东西：不铺 preset 也照样有皮肤，只是没有莫宁的口吻。
 */
import { createReadStream, statSync, readFileSync, writeFileSync, existsSync, mkdirSync, readdirSync, cpSync } from 'node:fs'
...
/* ---------- 莫宁人格 preset ----------
 * 随插件携带 preset/（agent.cordis.yml + preset.yml），幂等铺设到 DSH 的 preset 目录。
 * 装完皮肤后**新建会话**，在 preset 选择器里选「莫宁」，就是莫宁的口吻在工作。
 * 目录已存在就跳过 —— 绝不覆盖用户改过的那份（要强制覆盖用 DSH_MORNYE_REDEPLOY_PRESET=1）。 */
const DSH_HOME =
  process.env.DSH_HOME || join(process.env.APPDATA || process.env.HOME || '.', 'dsh-desktop', 'harness')
const PRESET_ID = 'mornye'
const PRESET_SRC = join(ROOT, 'preset')
const PRESET_DEST = join(DSH_HOME, '.agent-presets', PRESET_ID)

/** 幂等铺设人格 preset。任何失败都只记录、不抛出 —— 皮肤不该因为人格装不上而挂掉。 */
function deployPreset() {
  const st = { src: PRESET_SRC, dest: PRESET_DEST, installed: false, present: false, skipped: null, files: [], error: null }
  try {
    if (process.env.DSH_MORNYE_SKIP_PRESET === '1') {
      st.skipped = 'DSH_MORNYE_SKIP_PRESET=1'
      return st
    }
    if (!existsSync(join(PRESET_SRC, 'agent.cordis.yml'))) {
      st.skipped = 'preset/agent.cordis.yml 不存在'
      return st
    }
    st.files = readdirSync(PRESET_SRC).filter((f) => f.endsWith('.yml'))
    if (existsSync(PRESET_DEST) && process.env.DSH_MORNYE_REDEPLOY_PRESET !== '1') {
      st.present = true
      st.skipped = '目标已存在，未覆盖（想强制覆盖：DSH_MORNYE_REDEPLOY_PRESET=1）'
      return st
    }
    mkdirSync(PRESET_DEST, { recursive: true })
    for (const f of st.files) cpSync(join(PRESET_SRC, f), join(PRESET_DEST, f))
    st.installed = true
    st.present = true
  } catch (e) {
...
function readFlag() {
  ...
  return {
    ...
    /* 余额桌宠的显示设置：数据源（官网余额 / 订阅额度）+ 挂件缩放（0.7~1.6） */
    petSource: raw.petSource === 'opencode-go' ? 'opencode-go' : 'deepseek',
    petScale: readJsonNumber(raw.petScale) ? Math.max(0.7, Math.min(1.6, raw.petScale)) : 1,
  }
}
...
function handleFlag(req, res) {
  ...
  if (req.method === 'POST') {
    readBody(req, res, (value) => {
      ...
      /* 桌宠显示设置：数据源与缩放（越界值夹回来，脏数据丢掉） */
      if (value.petSource === 'deepseek' || value.petSource === 'opencode-go') next.petSource = value.petSource
      if (readJsonNumber(value.petScale)) {
        next.petScale = Math.max(0.7, Math.min(1.6, value.petScale))
      }
...
export function apply(ctx) {
  /* 人格 preset：随插件幂等铺设到 DSH 的 preset 目录（改人格文件后新建会话才生效） */
  const presetStatus = deployPreset()
  if (presetStatus.error) console.warn('mornye-theme: preset deploy failed:', presetStatus.error)
  ...
  ctx.inject(['webServer'], (hostCtx) => {
    hostCtx.effect(() => {
      ...
      const disposers = [
        ...
        /* 人格 preset 的铺设状态：装没装、铺到哪个目录（排错用） */
        hostCtx.webServer.register({
          kind: 'exact',
          path: `${ROUTE_PREFIX}/preset`,
          handler: (req, res) => sendJson(res, 200, presetStatus),
        }),
      ]
      return () => {
